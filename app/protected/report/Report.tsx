...
export default function Report() {
      ...
      if (char === ' ' || i === text.length - 1) {
        if (currentLine.length + currentWord.length > maxLineLength) {
          if (currentWord.length > maxLineLength) {
            while (currentWord.length > maxLineLength) {
              lines.push(currentWord.slice(0, maxLineLength - 1) + '-');
              currentWord = currentWord.slice(maxLineLength - 1);
            }
          } else {
  ...
  const justifyText = (doc: jsPDF, text: string, x: number, y: number, maxWidth: number, lineHeight: number) => {
    ...
    lines.forEach((line, index) => {
      if (index < lines.length - 1) {
        ...
        doc.text(justifiedLine, x, y + lineHeight * index);
      } else {
        doc.text(line, x, y + lineHeight * index);
      }
    });
  };
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    dominioData.forEach((entry: any, index: number) => {
      ...
      doc.setFontSize(10);
      const regNameX = (doc.internal.pageSize.getWidth() - doc.getTextWidth(regName.toUpperCase())) / 2;
      doc.text(regName.toUpperCase(), regNameX, 200);
      ...
      doc.save(`CERTIFICADO_HISTORIA_DOMINIO_${index + 1}.PDF`);
    });
  ...
  return (
    <div className="max-w mx-auto p-8 bg-white relative">
      <h1 className="text-2xl font-bold mb-4 text-center">Reporte</h1>
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div>
          ...
          />
          <label className="block text-sm font-medium text-gray-700 pt-2">Nombre Registrador:</label>
          <input
            type="text"
            name="regName"
            value={regName}
